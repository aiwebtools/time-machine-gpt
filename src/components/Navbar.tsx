...
const Navbar = () => {
  ...
  return <header className={cn("fixed top-0 w-full z-50 transition-all duration-300", scrollPosition > 50 ? "bg-white/80 backdrop-blur-md shadow-sm py-3" : "bg-transparent py-6")}>
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <button className="md:hidden" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("transition-colors", scrollPosition > 50 ? "text-time-dark" : "text-time-accent")}>
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          <a href="https://www.aiwebtools.ai" target="_blank" rel="noopener noreferrer" className={cn("font-medium transition-colors", "hover:text-time-accent", scrollPosition > 50 ? "text-time-dark" : "text-time-accent")}>
            MORE AI TOOLS
          </a>
      ...
      <div className={cn("md:hidden absolute w-full left-0 px-4 py-3 shadow-lg", "bg-white/95 backdrop-blur-md", "transition-all duration-300 ease-in-out", isMenuOpen ? "opacity-100 top-full" : "opacity-0 -top-40 pointer-events-none")}>
        <nav className="flex flex-col space-y-3 text-time-dark">
          ...
          <a href="https://www.aiwebtools.ai" target="_blank" rel="noopener noreferrer" className="w-full py-2 hover:bg-gray-100 px-2 rounded text-time-dark transition-colors" onClick={() => setIsMenuOpen(false)}>
            MORE AI TOOLS
          </a>
          <a href={HISTORY_GPT_URL} target="_blank" rel="noopener noreferrer" className="w-full py-3 text-center bg-time-dark text-white rounded-md mt-2 hover:bg-time-dark/80 transition-colors" onClick={() => setIsMenuOpen(false)}>
            TALK TO HISTORY GPT
          </a>
          <a href={TIME_MACHINE_URL} target="_blank" rel="noopener noreferrer" className="w-full py-3 text-center bg-time-medium text-white rounded-md mt-2 hover:bg-time-dark transition-colors" onClick={() => setIsMenuOpen(false)}>
            Begin Journey
          </a>
        </nav>
      </div>
