...
const Features = ({ className, addToRefs }: FeaturesProps) => {
  ...
  return (
    <section className={cn("py-12 md:py-20 bg-time-dark text-white", className)}>
      <div className="container mx-auto px-4 md:px-6">
        <div ref={addToRefs} className="reveal text-center max-w-3xl mx-auto mb-8 md:mb-16">
          ...
          <h2 className="text-2xl md:text-4xl font-serif font-bold mb-4 md:mb-6 text-glow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              ref={addToRefs}
              className="reveal backdrop-blur-sm bg-time-dark/40 border border-time-accent/10 p-4 md:p-6 rounded-xl transition-all duration-300 hover:bg-time-medium/30 hover:border-time-accent/30 hover:shadow-[0_0_15px_rgba(194,160,110,0.3)] hover:scale-105 group"
            >
              <div className="flex items-start space-x-3 md:space-x-4">
                ...
                <div>
                  <h3 className="text-lg md:text-xl font-medium mb-2 text-time-accent group-hover:text-time-accent/90 transition-colors">
                    {feature.title}
                  </h3>
                  <p className="text-white/70 group-hover:text-white/90 transition-colors text-sm md:text-base">
                    {feature.description}
                  </p>
                </div>
              </div>
